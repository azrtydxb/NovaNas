...
func TestBondReconciler_HappyPath(t *testing.T) {
	...
	if got.Status.Phase != "Ready" && got.Status.Phase != "Active" {
		t.Fatalf("phase = %q, want Ready|Active", got.Status.Phase)
	}
}
...
func TestVlanReconciler_HappyPath(t *testing.T) {
	...
	if got.Status.Phase != "Ready" && got.Status.Phase != "Applied" &&
		got.Status.Phase != "Active" && got.Status.Phase != "Connected" {
...
func TestVipPoolReconciler_HappyPath(t *testing.T) {
	...
	if got.Status.Phase != "Ready" && got.Status.Phase != "Pending" &&
		got.Status.Phase != "Active" && got.Status.Phase != "Applied" {
		t.Fatalf("phase = %q", got.Status.Phase)
	}
}
