...
type ApiTokenSpec struct {
	...
	// ExpiresAt is an optional absolute expiry. Nil means no expiry.
	// +optional
	...
	// Must be a valid Go duration string (e.g. "720h"). Empty disables
	// automatic rotation.
	// +optional
...
type ApiTokenStatus struct {
	...
	// SecretRef names the Secret holding the token hash.
	// +optional
