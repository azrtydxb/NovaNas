...
export function CapacityBar({ used, total, label, className }: CapacityBarProps) {
  ...
  return (
    <div className={cn('grid grid-cols-[80px_1fr_80px] items-center gap-2 min-w-[220px]', className)}>
      ...
      <span className='h-1.5 bg-elevated rounded-full overflow-hidden'>
        <span className={cn('block h-full transition-[width] duration-500', tone)} style={{ width: `${pct}%` }} />
      </span>
      <span className='mono text-xs text-foreground-muted text-right'>
        {formatBytes(total)}
      </span>
    </div>
  );
