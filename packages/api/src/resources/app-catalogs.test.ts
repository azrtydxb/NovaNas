...
describe('app-catalogs resource', () => {
  let h: TestAppHandle;
  let adminSid: string;
  let userSid: string;
  ...
  it('admin CRUDs', async () => {
    const obj = { ...sample, metadata: { name: 'cat-b' } };
  ...
  it('404 on missing', async () => {
    const r = await h.built.app.inject({
      method: 'GET',
      url: '/api/v1/app-catalogs/missing',
      headers: { cookie: cookieFor(h.built, adminSid) },
    });
