...
const sample = {
  ...
  kind: 'NvmeofTarget',
  metadata: { name: 'nvme-a' },
  spec: {
    blockVolume: 'vol-a',
    listen: { hostInterface: 'eth0' },
  },
};

describe('nvmeof-targets resource', () => {
  let h: TestAppHandle;
  let adminSid: string;
  let userSid: string;
  ...
  it('admin CRUDs', async () => {
    const obj = { ...sample, metadata: { name: 'nvme-b' } };
    ...
    const d = await h.built.app.inject({
      method: 'DELETE',
      url: '/api/v1/nvmeof-targets/nvme-b',
      headers: { cookie: cookieFor(h.built, adminSid) },
    });
  ...
  it('404 on missing', async () => {
    const r = await h.built.app.inject({
      method: 'GET',
      url: '/api/v1/nvmeof-targets/missing',
      headers: { cookie: cookieFor(h.built, adminSid) },
    });
