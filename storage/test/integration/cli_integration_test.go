...
// buildCLI builds the novanasctl binary for testing.
func buildCLI(t *testing.T) string {
	t.Helper()

	// Find the repository root by looking for go.mod
	repoRoot := "../.."
	binaryPath := t.TempDir() + "/novanasctl"
...
// runCLI runs the novanasctl binary with the given args and returns the output.
...
func TestCLI_Version(t *testing.T) {
	...
	if !strings.Contains(out, "novanasctl") {
		t.Errorf("version output does not contain 'novanasctl': %s", out)
	}
}
...
func TestCLI_Completion(t *testing.T) {
	...
	if !strings.Contains(out, "complete -o default -F __start_novanasctl novanasctl") {
	...
	if !strings.Contains(out, "#compdef novanasctl") {
