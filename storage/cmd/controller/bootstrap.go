...
func runTLSBootstrap() error {
	...
	// Include wildcard patterns for FQDN resolution (*.novanas-system.svc)
	// and explicit short names for same-namespace resolution.
	releaseName := strings.TrimSuffix(*secretName, "-tls-certs")
	serverCertPEM, serverKeyPEM, err := transport.GenerateCert(caCertPEM, caKeyPEM, "novanas-server", true, []string{
		"localhost",
