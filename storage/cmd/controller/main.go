...
import (
	...
	"github.com/azrtydxb/novanas/storage/internal/operator"
	"github.com/azrtydxb/novanas/storage/internal/policy"
...
func main() {
	...
	if showVersion {
		_, _ = fmt.Fprintf(os.Stdout, "novanas-storage-controller %s (commit: %s, built: %s)\n", version, commit, date)
		os.Exit(0)
	}

	shutdownTracer := observability.InitTracer("novanas-storage-controller", logging.L)
	defer shutdownTracer()

	ctrl.SetLogger(crzap.New(crzap.UseFlagOptions(&opts)))
	setupLog := ctrl.Log.WithName("setup")

	setupLog.Info("starting novanas-storage-controller", "version", version, "commit", commit, "date", date)
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "novanas-storage-controller-leader-election",
