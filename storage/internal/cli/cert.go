...
func runCertBootstrap() error {
	...
	serverDNSNames := []string{
		...
		// Kubernetes service discovery SANs for novanas-system namespace
		"novanas-storage-meta",
		"novanas-storage-meta-headless",
		"novanas-storage-agent",
		"*.novanas-storage-meta-headless.novanas-system.svc.cluster.local",
		"*.novanas-system.svc.cluster.local",
	}
...
func generateSecretManifest(caCert, caKey, serverCert, serverKey, clientCert, clientKey []byte) (string, error) {
	...
	buf.WriteString("  name: novanas-tls-certs\n")
	buf.WriteString("  namespace: novanas\n")
