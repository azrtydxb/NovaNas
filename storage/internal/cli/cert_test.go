...
func TestCertBootstrap(t *testing.T) {
	...
	serverCertPEM, serverKeyPEM, err := transport.GenerateCert(caCertPEM, caKeyPEM, "novanas-server", true, serverDNSNames)
	if err != nil {
	...
	clientCertPEM, clientKeyPEM, err := transport.GenerateCert(caCertPEM, caKeyPEM, "novanas-client", false, nil)
	if err != nil {
...
func TestCertBootstrapExistingCA(t *testing.T) {
	...
	serverCertPEM, _, err := transport.GenerateCert(readCACertPEM, readCAKeyPEM, "novanas-server", true, []string{"localhost"})
	if err != nil {
...
func TestCertValidity(t *testing.T) {
	...
	// Generate server certificate
	serverCertPEM, _, err := transport.GenerateCert(caCertPEM, caKeyPEM, "novanas-server", true, []string{"localhost"})
	if err != nil {
		t.Fatalf("generating server certificate: %v", err)
	}
	...
	// Generate client certificate
	clientCertPEM, _, err := transport.GenerateCert(caCertPEM, caKeyPEM, "novanas-client", false, nil)
	if err != nil {
		t.Fatalf("generating client certificate: %v", err)
	}
...
func TestMTLSComponentCommunication(t *testing.T) {
	...
	// Generate server certificate
	serverCertPEM, serverKeyPEM, err := transport.GenerateCert(caCertPEM, caKeyPEM, "novanas-server", true, []string{"localhost"})
	if err != nil {
		t.Fatalf("generating server certificate: %v", err)
	}
