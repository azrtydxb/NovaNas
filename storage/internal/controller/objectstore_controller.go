...
const (
	defaultS3GatewayImage = "ghcr.io/azrtydxb/novanas/novanas-storage-s3gw:v0.1.0"
...
// +kubebuilder:rbac:groups=novanas.io,resources=objectstores/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=novanas.io,resources=objectstores/finalizers,verbs=update
...
func (r *ObjectStoreReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	secretName := fmt.Sprintf("novanas-s3-%s-keys", store.Name)
	...
	deployName := fmt.Sprintf("novanas-s3-%s", store.Name)
	...
	svcName := fmt.Sprintf("novanas-s3-%s", store.Name)
...
func (r *ObjectStoreReconciler) reconcileS3Secret(ctx context.Context, store *novastorev1alpha1.ObjectStore, name string) error {
	...
	_, err := controllerutil.CreateOrUpdate(ctx, r.Client, secret, func() error {
		...
		secret.Labels["app.kubernetes.io/managed-by"] = "novanas-storage-controller"
...
func (r *ObjectStoreReconciler) reconcileS3Deployment(ctx context.Context, store *novastorev1alpha1.ObjectStore, name, secretName string) error {
	...
	_, err := controllerutil.CreateOrUpdate(ctx, r.Client, deploy, func() error {
		appLabels := map[string]string{
			"app.kubernetes.io/name":       "novanas-s3-gateway",
