...
import (
	...
	pb "github.com/azrtydxb/novanas/storage/api/proto/nvme"
...
func (s *SPDKTargetServer) CreateTarget(ctx context.Context, req *pb.CreateTargetRequest) (*pb.CreateTargetResponse, error) {
	_, span := otel.Tracer("novanas-storage-agent").Start(ctx, "SPDKTargetServer.CreateTarget")
	defer span.End()
	...
	// named "novanas_<volumeID>" that bridges block I/O to the chunk engine.
	...
	if s.skipNVMeF {
		nqn := fmt.Sprintf("nqn.2024-01.io.novanas:volume-%s", volumeID)
...
func (s *SPDKTargetServer) DeleteTarget(ctx context.Context, req *pb.DeleteTargetRequest) (*pb.DeleteTargetResponse, error) {
	...
	// This destroys the novanas_<volumeID> virtual bdev and its chunk map.
