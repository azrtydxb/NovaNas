...
//!
//! Each volume is registered as an SPDK bdev named `novanas_<volume_id>`.
...
pub fn set_metadata_store(store: Arc<MetadataStore>) {
    if METADATA_STORE.set(store).is_err() {
        info!("novanas_bdev: metadata store already set, ignoring");
    }
}
...
pub fn allocate_volume_offset(volume_name: &str, size_bytes: u64) -> u64 {
    ...
    info!(
        "novanas_bdev: allocated volume '{}' at bdev offset {} ({}MB)",
        volume_name,
        base,
        base / (1024 * 1024)
    );
...
pub fn get_volume_base_offset(volume_name: &str) -> Result<u64> {
    ...
    info!(
        "novanas_bdev: lazy-allocated volume '{}' at offset {}",
        volume_name, offset
    );
    Ok(offset)
}
...
pub fn set_backend_bdev_name(name: &str) {
    if BACKEND_BDEV_NAME.set(name.to_string()).is_err() {
        info!("novanas_bdev: backend bdev name already set, ignoring");
    }
}
...
pub fn load_chunk_maps_from_store() {
    let store = match get_metadata_store() {
        Some(s) => s,
        None => {
            warn!("novanas_bdev: load_chunk_maps_from_store called but no metadata store set");
            return;
        }
    };
    ...
    let volumes = match store.list_volumes() {
        ...
        Err(e) => {
            warn!(
                "novanas_bdev: failed to list volumes from metadata store: {}",
                e
            );
            return;
        }
    ...
    if volumes.is_empty() {
        info!("novanas_bdev: no persisted volumes found in metadata store");
        return;
    }
    ...
    for vol in &volumes {
        ...
        let entries = match store.list_chunk_map(&vol.id) {
            ...
            Err(e) => {
                warn!(
                    "novanas_bdev: failed to load chunk map for volume '{}': {}",
                    vol.id, e
                );
                continue;
            }
        ...
        for entry in &entries {
            ...
            if idx < chunk_map.len() {
                ...
                warn!(
                    "novanas_bdev: chunk index {} exceeds volume '{}' chunk count {}",
    ...
    info!(
        "novanas_bdev: loaded {} chunk map entries across {} volumes from metadata store",
        total_entries,
        volumes.len()
    );
}
...
pub fn set_chunk_engine(engine: Arc<ChunkEngine>, handle: tokio::runtime::Handle) {
    if CHUNK_ENGINE.set(engine).is_err() {
        info!("novanas_bdev: chunk engine already set, ignoring");
    }
...
fn cleanup_volume_locks(volume_name: &str) {
    if let Some(locks) = SUB_BLOCK_LOCKS.get() {
        ...
        if removed > 0 {
            info!(
                "novanas_bdev: cleaned up {} sub-block locks for volume '{}'",
                removed, volume_name
            );
        }
    }
...
fn update_dirty_bitmap(volume_name: &str, chunk_idx: usize, sb_idx: usize) -> Result<()> {
    ...
    if let Ok(registry) = bdev_registry().lock() {
        let bdev_key = format!("novanas_{}", volume_name);
...
/// bdev named `novanas_<volume_name>` with the given size. The bdev can then
...
pub fn create(volume_name: &str, size_bytes: u64) -> Result<String> {
    ...
    let bdev_name = format!("novanas_{}", volume_name);
    ...
    if let Some(store) = get_metadata_store() {
        ...
        if let Err(e) = store.put_volume(&vol) {
            warn!(
                "novanas_bdev: failed to persist volume definition '{}': {}",
                volume_name, e
            );
        }
    }
    ...
    info!(
        "novanas_bdev: creating bdev '{}' for volume '{}' ({} blocks)",
    ...
    let (bdev_addr, registered_ctx_addr) =
        reactor_dispatch::dispatch_sync(move || -> Result<(usize, usize)> {
            unsafe {
                ...
                let io_dev_name =
                    std::ffi::CString::new(format!("novanas_io_{}", bdev_name_clone)).unwrap();
                ffi::spdk_io_device_register(
                    ctx_ptr as *mut c_void,
                    Some(novanas_channel_create_cb),
                    Some(novanas_channel_destroy_cb),
                ...
                (*bdev).module = novanas_bdev_module_ptr();
    ...
    info!(
        "novanas_bdev: registered bdev '{}' (volume='{}', size={}B)",
    ...
    Ok(format!("novanas_{}", volume_name))
}
...
pub fn destroy(volume_name: &str) -> Result<()> {
    ...
    if let Some(maps) = VOLUME_CHUNK_MAPS.get() {
        ...
        if map.remove(volume_name).is_some() {
            info!(
                "novanas_bdev: cleaned up chunk map for volume '{}'",
                volume_name
            );
        }
    }

    info!("novanas_bdev: destroyed bdev for volume '{}'", volume_name);
    Ok(())
}
...
fn novanas_bdev_module_ptr() -> *mut ffi::spdk_bdev_module {
...
unsafe extern "C" fn novanas_channel_create_cb(_io_device: *mut c_void, _ctx: *mut c_void) -> i32 {
    0
}

/// No-op channel destroy callback.
unsafe extern "C" fn novanas_channel_destroy_cb(_io_device: *mut c_void, _ctx: *mut c_void) {}
...
unsafe extern "C" fn bdev_submit_request_cb(
    ...
    match io_type {
        ffi::spdk_bdev_io_type_SPDK_BDEV_IO_TYPE_READ => {
            ...
            // Reactor NDP reads — zero crossing for remote reads.
        ...
        ffi::spdk_bdev_io_type_SPDK_BDEV_IO_TYPE_WRITE => {
            ...
            match engine.write_cache.absorb(&volume_name, offset, &data) {
                ...
                AbsorbResult::NotAligned => {
                    ...
                    // Try reactor-native local write first.
                    ...
                    handle.spawn(async move {
                        ...
                        let status = match result {
                            ...
                            Err(e) => {
                                error!("novanas_bdev: write (not aligned) failed: {}", e);
                                ffi::spdk_bdev_io_status_SPDK_BDEV_IO_STATUS_FAILED
                            }
                        };
                        reactor_dispatch::send_to_reactor(move || unsafe {
                ...
                AbsorbResult::Full => {
                    ...
                    handle.spawn(async move {
                        ...
                        let status = match result {
                            Ok(()) => ffi::spdk_bdev_io_status_SPDK_BDEV_IO_STATUS_SUCCESS,
                            Err(e) => {
                                error!("novanas_bdev: write (cache full) failed: {}", e);
                                ffi::spdk_bdev_io_status_SPDK_BDEV_IO_STATUS_FAILED
                            }
                        };
                        reactor_dispatch::send_to_reactor(move || unsafe {
                        ...
                        if !pending.is_empty() {
                            ...
                            while let Some(result) = tasks.join_next().await {
                                if let Ok(Err(e)) = result {
                                    error!("novanas_bdev: pending write flush failed: {}", e);
                                    flush_ok = false;
                                }
                            }
                        }
        ...
        ffi::spdk_bdev_io_type_SPDK_BDEV_IO_TYPE_COPY => {
            ...
            handle.spawn(async move {
                ...
                let status = match result {
                    Ok(()) => ffi::spdk_bdev_io_status_SPDK_BDEV_IO_STATUS_SUCCESS,
                    Err(e) => {
                        error!("novanas_bdev: copy failed: {}", e);
                        ffi::spdk_bdev_io_status_SPDK_BDEV_IO_STATUS_FAILED
                    }
                };
                reactor_dispatch::send_to_reactor(move || unsafe {
