...
//!
//! This follows the same pattern as `novanas_bdev.rs` (the ChunkEngine
...
pub fn create(replica: Arc<ReplicaBdev>, size_bytes: u64) -> Result<String> {
    ...
    info!(
        "novanas_replica_bdev: creating bdev '{}' for volume '{}' ({} blocks)",
    ...
    let (bdev_addr, registered_ctx_addr) =
        reactor_dispatch::dispatch_sync(move || -> Result<(usize, usize)> {
            unsafe {
                ...
                let io_dev_name =
                    std::ffi::CString::new(format!("novanas_replica_io_{}", bdev_name_clone))
                        .unwrap();
    ...
    info!(
        "novanas_replica_bdev: registered bdev '{}' (volume='{}', size={}B)",
...
pub fn destroy(volume_id: &str) -> Result<()> {
    ...
    info!(
        "novanas_replica_bdev: destroying bdev for volume '{}'",
        volume_id
    );
    ...
    info!(
        "novanas_replica_bdev: destroyed bdev for volume '{}'",
        volume_id
    );
    Ok(())
}
...
fn replica_bdev_module_ptr() -> *mut ffi::spdk_bdev_module {
    REPLICA_MODULE_INIT.call_once(|| unsafe {
        REPLICA_MODULE.name = b"novanas_replica\0".as_ptr() as *const c_char;
...
unsafe extern "C" fn module_init_cb() -> i32 {
    info!("novanas_replica_bdev: module initialised");
    0
}

unsafe extern "C" fn module_fini_cb() {
    info!("novanas_replica_bdev: module shutdown");
}
        ...
        ffi::spdk_bdev_io_type_SPDK_BDEV_IO_TYPE_READ => {
            ...
            io_pool().execute(move || {
                ...
                match result {
                    ...
                    Err(e) => {
                        error!("novanas_replica_bdev: read failed: {}", e);
                        reactor_dispatch::send_to_reactor(move || unsafe {
                            ffi::spdk_bdev_io_complete(
                                bdev_io_addr as *mut ffi::spdk_bdev_io,
                                ffi::spdk_bdev_io_status_SPDK_BDEV_IO_STATUS_FAILED,
        ...
        ffi::spdk_bdev_io_type_SPDK_BDEV_IO_TYPE_WRITE => {
            ...
            io_pool().execute(move || {
                ...
                let status = match result {
                    ...
                    Err(e) => {
                        error!("novanas_replica_bdev: write failed: {}", e);
                        ffi::spdk_bdev_io_status_SPDK_BDEV_IO_STATUS_FAILED
                    }
                };
