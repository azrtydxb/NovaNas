...
use crate::bdev::novanas_bdev::{get_metadata_store, volume_chunk_maps};
...
async fn sync_all_volumes(
    ...
    crate::bdev::novanas_bdev::sync_atomic_bitmaps_to_chunk_maps();
...
async fn sync_one_chunk(
    ...
    if let Ok(registry) = crate::bdev::novanas_bdev::bdev_registry().lock() {
        for entry in registry.values() {
            let ctx = entry.ctx_ptr as *const crate::bdev::novanas_bdev::BdevCtx;
            if !ctx.is_null() {
