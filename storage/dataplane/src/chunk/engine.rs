...
impl ChunkEngine {
                ...
                if let Ok(Some(cm_entry)) = store.get_chunk_map(volume_id, chunk_index) {
                    for placement_node in &cm_entry.placements {
                        ...
                        let result = if placement_node == &self.node_id {
                            crate::bdev::novanas_bdev::sub_block_read_local(
        ...
        for (target_node, _backend) in &placements {
            ...
            join_set.spawn(async move {
                if target_owned == node_id {
                    ...
                    #[cfg(feature = "spdk-sys")]
                    {
                        crate::bdev::novanas_bdev::sub_block_write_local(
                            &volume_id_owned,
                            write_offset,
                            &data_owned,
                        )
    ...
    pub async fn migrate_chunk(
        ...
        let data = crate::bdev::novanas_bdev::sub_block_read_local(
            volume_id,
            chunk_offset,
            CHUNK_SIZE as u64,
        )
...
mod tests {
    ...
    async fn write_records_policy_location_and_ref() {
        ...
        let store = Arc::new(
            FileChunkStore::new(chunk_dir.path().to_path_buf(), 64 * 1024 * 1024).unwrap(),
        );
        let loc_store =
