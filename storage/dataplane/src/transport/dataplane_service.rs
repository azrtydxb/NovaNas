...
fn volume_id_to_nqn(volume_id: &str) -> String {
    format!("nqn.2024-01.io.novanas:volume-{}", volume_id)
}
...
impl DataplaneService for DataplaneServiceImpl {
    ...
    async fn init_chunk_store(
        ...
        // Wire up the ChunkEngine with PolicyEngine for the novanas bdev module.
        ...
        // RPCs can reach it.
        {
            ...
            let policy_db_dir = format!("/var/lib/novanas/policy");
            ...
            crate::bdev::novanas_bdev::set_chunk_engine(engine, handle);
            crate::bdev::novanas_bdev::set_backend_bdev_name(&req.bdev_name);
            ...
            let metadata_dir = format!("/var/lib/novanas/metadata");
            if let Err(e) = std::fs::create_dir_all(&metadata_dir) {
                ...
                match crate::metadata::store::MetadataStore::open(&db_path) {
                    Ok(store) => {
                        ...
                        crate::bdev::novanas_bdev::set_metadata_store(store);
                        crate::bdev::novanas_bdev::load_chunk_maps_from_store();
                        log::info!(
    ...
    async fn put_sub_block(
        ...
        let req = request.into_inner();
        let bdev_name = crate::bdev::novanas_bdev::get_backend_bdev_name()
    ...
    async fn create_volume(
        ...
        if req.frontend_only {
            if crate::bdev::novanas_bdev::get_chunk_engine().is_err() {
                log::info!(
        ...
        // ALL volumes go through novanas_bdev → ChunkEngine → Backend.
        ...
        // The novanas_bdev creates a virtual SPDK bdev.
        let bdev = crate::bdev::novanas_bdev::create(&req.name, req.size_bytes)
            .map_err(|e| Status::internal(format!("create novanas bdev: {e}")))?;
        ...
        if let Ok(engine) = crate::bdev::novanas_bdev::get_chunk_engine() {
    ...
    async fn delete_volume(
        ...
        let req = request.into_inner();
        // ALL volumes are novanas_bdevs on top of ChunkEngine.
        crate::bdev::novanas_bdev::destroy(&req.name)
            .map_err(|e| Status::internal(format!("destroy novanas bdev: {e}")))?;
        // Clean up the chunk map for this volume (in-memory and persistent).
        if let Some(maps) = crate::bdev::novanas_bdev::volume_chunk_maps_ref() {
            maps.write().unwrap().remove(&req.name);
        }
        if let Some(store) = crate::bdev::novanas_bdev::metadata_store_ref() {
    ...
    async fn set_volume_policy(
        ...
        if let Ok(chunk_engine) = crate::bdev::novanas_bdev::get_chunk_engine() {
    ...
    async fn update_topology(
        ...
        let engine = crate::bdev::novanas_bdev::get_chunk_engine()
        ...
        if accepted {
            ...
            // Connect reactor NDP peers (testing connections only, no I/O).
    ...
    async fn get_chunk_maps(
        ...
        let maps = crate::bdev::novanas_bdev::volume_chunk_maps()
            .read()
            .unwrap();
    ...
    async fn sync_chunk_maps(
        ...
        if let Some(store) = crate::bdev::novanas_bdev::get_metadata_store() {
