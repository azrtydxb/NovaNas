// Package v1alpha1 contains API Schema definitions for the novanas project.
// +k8s:deepcopy-gen:package,register
// nolint:revive // Auto-generated by controller-runtime and kubebuilder; comments are in GoDoc markers
//
// CRD consolidation status (issue #35, docs/CRD-CONSOLIDATION-PLAN.md):
//
// The plan calls for re-exporting these types as Go type aliases of the
// authoritative definitions in packages/operators/api/v1alpha1. On
// inspection, the two packages' Spec/Status shapes have diverged
// substantially:
//
//   - StoragePoolSpec: storage uses NodeSelector/BackendType/FileBackend
//     (an implementation contract with backend drivers); operators uses
//     Tier/RecoveryRate/RebalanceOnAdd (a policy contract).
//   - DeviceFilter: storage uses Type/MinSize; operators uses
//     PreferredClass/MinSize/MaxSize.
//   - BlockVolumeSpec: storage uses AccessMode/DataProtection/Quota;
//     operators uses Protection/Tiering. ErasureCodingSpec field types
//     differ (int vs int32) — not structurally aliasable.
...
// directive in storage/go.mod is in place so a follow-up can perform
// a field-by-field reconciliation (additive changes on the operators
// side first, then aliases) without further module plumbing. The
// "Executed" section of the plan tracks the outcome.
package v1alpha1
...
type FileBackendSpec struct {
	...
	// +kubebuilder:default="/var/lib/novanas/file"
