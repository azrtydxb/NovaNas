...
- `User` and `Group` CRDs are **projections** of Keycloak users/groups, synced by a `keycloak-sync` operator
- Source of truth = Keycloak; CRDs exist only so K8s RBAC and audit can reference them
- `KeycloakRealm` / `KeycloakFederationProvider` CRDs wrap Keycloak realm config (federation providers, custom claims, etc.)
...
- Upstream OpenBao (Vault fork) via Helm subchart
...
- Admin-provided **recovery passphrase** is the DR path: unseal keys are exported encrypted with this passphrase and included in `ConfigBackupPolicy`
...
Each pod authenticates via its Kubernetes ServiceAccount (OpenBao's `kubernetes` auth method) and receives a scoped token matching its workload identity.

### Chunk engine integration

- Chunk engine SA binds to OpenBao `kubernetes` auth method
...
## Secret reference pattern in CRDs

CRDs that need secrets use explicit OpenBao paths, not K8s Secret names:
...
An **OpenBao Agent Injector** (sidecar pattern, standard in Vault/OpenBao) translates these at pod admission into mounted secret files inside the pod. Secrets never written to K8s Secret objects.

Exception: bootstrap secrets (OpenBao's own seal context, initial Postgres password before OpenBao is up) live in K8s Secrets. These are minimal and rotated once the system is up.
...
```yaml
kind: Certificate
metadata: { name: plex-cert }
spec:
  dnsNames: [plex.nas.local]
  issuer:
    type: internal-pki              # internal-pki | acme | uploaded
    pkiPath: novanas/pki/services
  duration: 90d
  renewBefore: 30d
status:
  secretRef: openbao://novanas/certs/plex-cert
  notBefore: ...
  notAfter: ...
```

novaedge reads key material from OpenBao (preferred) or receives short-lived K8s Secrets synced from OpenBao by the `novanas-cert-sync` operator.

## KmsKey CRD (SSE-KMS)

```yaml
kind: KmsKey
metadata: { name: finance-sse-kms }
spec:
  description: "Finance department SSE-KMS key"
  keyMaterial:
    source: tpm            # tpm | passphrase | external
  rotation:
    enabled: true
    intervalDays: 365
```
...
| `TwoFactor` CRD | Keycloak built-in |
| `IdentityProvider` CRD as code | `KeycloakRealm` config CRD |
| K8s Secrets for TLS / DK / tokens | OpenBao paths |
